...
### 修复
- **翻牌前大盲注加注权（BB option）**：大盲不再被错误标记为已行动，可在翻牌前补行动（加注/跟注/弃牌）；无人加注时可 check 结束翻牌前。附带修复 `raise`/`allin` 分支漏设 `acted` 导致加注后轮次不收敛的问题
- **摊牌后自动连发下一局**：房主点"继续"不再自动立刻发牌。改为二次确认——房主需在 3 秒内再次点击"确认开始下一局？"才发牌，防止误触导致无限循环（筹码保留，无限记账）

### 测试
- 新增 4 项翻牌前大盲注加注权测试（现共 31 项）
